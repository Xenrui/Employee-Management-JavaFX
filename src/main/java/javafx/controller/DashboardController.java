...
public class DashboardController implements Initializable {
    ...
    @FXML
    private DatePicker meetings_DatePicker;
    ...
    public void addProject(ActionEvent event){ //calls addProject.fxml to add new Project
        try {
            Stage stage = new Stage();
    ...
    private VBox createProjectCard(Project project) { 
        ...
        // Edit button inside the header
        Button editButton = new Button();
        FontAwesomeIcon editIcon = new FontAwesomeIcon();
        editIcon.setGlyphName("EDIT");
        editButton.setGraphic(editIcon);
        editButton.setStyle("-fx-background-color: #ff9800; -fx-text-fill: white;");
        editButton.setOnAction(event -> editProject(project)); // Add your edit logic here
        StackPane.setAlignment(editButton, Pos.TOP_RIGHT); // Align button to the top-right
    
        // Add both project name and edit button to the header
        header.getChildren().addAll(projectNameLabel, editButton);
        ...
        card.getChildren().addAll(header, projectDescriptionLabel, startDateLabel, endDateLabel, viewDetailsButton);
    ...
    private void setDateMeeting() {
        // Set default value to today
        meetings_DatePicker.setValue(LocalDate.now());

        // A map to store scheduled meetings (Date -> Meeting Name)
        Map<LocalDate, String> meetings = new HashMap<>();
        meetings.put(LocalDate.of(2024, 11, 20), "Team Meeting");
        meetings.put(LocalDate.of(2024, 12, 10), "Project Deadline");

        // Customize the DatePicker's DayCellFactory
        meetings_DatePicker.setDayCellFactory(datePicker -> new DateCell() {
            @Override
            public void updateItem(LocalDate date, boolean empty) {
                super.updateItem(date, empty);

                // Mark dates with meetings
                if (meetings.containsKey(date)) {
                    this.setStyle("-fx-background-color: lightblue;");
                    this.setTooltip(new Tooltip(meetings.get(date))); // Add tooltip for the meeting name
                }
        ...
        // Add an event handler for date selection
        meetings_DatePicker.setOnAction(event -> {
            LocalDate date = meetings_DatePicker.getValue();
            if (meetings.containsKey(date)) {
                System.out.println("Selected Date: " + date + " (Meeting: " + meetings.get(date) + ")");
            } else {
                System.out.println("Selected Date: " + date + " (No Meetings Scheduled)");
            }
        });
    }
